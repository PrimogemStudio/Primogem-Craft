...
public class Wfj_sxProcedure {
	public static void execute(LevelAccessor world, Entity sourceentity, ItemStack itemstack) {
		...
		if (!world.isClientSide()) {
			...
			if (Math.random() < HSjinglianProcedure.execute(itemstack) * 0.2) {
				...
				if (!(sourceentity instanceof Player _plrCldCheck4 && _plrCldCheck4.getCooldowns().isOnCooldown(stack.getItem()))) {
					...
					if (sourceentity instanceof Player _player)
						_player.getCooldowns().addCooldown(PrimogemcraftModItems.WUFENGJIAN.get(), (int) (100 - 100 * HSjinglianProcedure.execute(itemstack) * 0.1));
				}
			} else {
				a = 0.02;
			}
