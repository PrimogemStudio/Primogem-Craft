...
public class Daima4shuxingProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		...
		if (!itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getBoolean("daishanchu") && !itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getBoolean("daima4_yixiufu")
				&& itemstack.getDamageValue() == 0) {
			{
				final String _tagName = "daima4_yixiufu";
		...
		if (itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getBoolean("daima4_yixiufu")) {
			if (!(entity instanceof LivingEntity _livEnt18 && _livEnt18.hasEffect(PrimogemcraftModMobEffects.JINGQUEDAIMA))) {
				if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
					_entity.addEffect(new MobEffectInstance(PrimogemcraftModMobEffects.JINGQUEDAIMA, 200, 0, false, false));
				NaijiuxianzhiProcedure.execute(itemstack);
			}
		}
	}
