...
public class Yiyijieguoshi_shuxingProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		...
		if (entity.getPersistentData().getBoolean("yijieguoshi_kaiqi")) {
			if (itemstack.getItem() == PrimogemcraftModItems.YIYIMUGUOSHI.get()) {
				{
			...
			if (itemstack.getDamageValue() == itemstack.getMaxDamage()) {
				...
				if (!(itemstack.getItem() == PrimogemcraftModItems.QWKWZG.get())) {
					DiaoyongqwyisunhuaiProcedure.execute(entity, itemstack);
				}
			}
		}
		if (!(entity instanceof LivingEntity _livEnt18 && _livEnt18.hasEffect(PrimogemcraftModMobEffects.QWYMGS)) && !(entity instanceof LivingEntity _livEnt19 && _livEnt19.hasEffect(PrimogemcraftModMobEffects.LINZHONG))) {
			if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
